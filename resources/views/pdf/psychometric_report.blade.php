...
{{-- ========================================================= --}}
<div class="border-b-2 border-indigo-600 pb-4 mb-8 flex justify-between items-end">
    <div>
...
{{-- ========================================================= --}}
{{-- INICIO DEL CÓDIGO DEL MODAL (TU UIX CUTE)                --}}
{{-- ========================================================= --}}
<div class="space-y-6">
    ...
    {{-- SCORECARD EJECUTIVO --}}
    <div class="relative overflow-hidden rounded-xl bg-white border border-gray-200 shadow-sm avoid-break">
    ...
    @if(count($dimensions) > 0 && isset($results['chart_type']) && $results['chart_type'] != 'radar')
        <div class="grid grid-cols-1 gap-6">
            ...
            @foreach($dimensions as $dimKey => $dimData)
                ...
                {{-- Añadimos la clase avoid-break para que no se corte en el PDF --}}
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                                @foreach($childSubscales as $subCode)
                                    @if(isset($subscales[$subCode]))
                                        ...
                                        @endphp
                                        <div class="bg-white p-2.5 rounded border border-gray-200 flex items-start gap-3 shadow-sm h-full avoid-break">
                                            ...
                                            <div class="flex-1 min-w-0">
                                                ...
                                                @if($subDescription)
                                                    <p class="text-[9px] text-gray-600 leading-tight mb-2">{{ $subDescription }}</p>
                                                @endif
                    ...
                    @if(isset($dimData['interpretation']) || isset($dimData['recommendation']))
                        <div class="p-4 bg-white grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                            @if(isset($dimData['interpretation']))
                                <div class="flex gap-3 avoid-break">
    ...
    {{-- FALLBACK PARA KOSTICK --}}
    @if(isset($results['chart_type']) && $results['chart_type'] == 'radar')
        <div class="grid grid-cols-4 md:grid-cols-5 gap-2 text-center avoid-break">
        ...
        <div class="p-3 bg-blue-50 text-blue-700 text-xs rounded border border-blue-100 flex items-center gap-2 justify-center">
            <span>ℹ️</span> Gráfica de Radar (Próximamente).
        </div>
    @endif
...
{{-- ========================================================= --}}
<div class="text-center mt-6 mb-4 text-xs font-semibold text-gray-400 uppercase tracking-widest border-t border-gray-200 pt-6">
